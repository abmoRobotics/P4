...
void full_automatic(){
    int length = sizeof(visionDataArray.msg)/sizeof(visionDataArray.msg[0]);
        for (size_t i = 0; i < length; i++){
            cout << "Object no." << i << "\n" << visionDataArray.msg[i].Class << endl;
        }
    ...
    for (size_t i = 0; i < length; i++) {
        ...
        goal.goalObject = visionDataArray.msg[i];
...
int main(int argc, char *argv[]){
    ...
    ros::NodeHandle nh;
    ros::Subscriber itongue_sub = nh.subscribe<jaco::RAWItongueOut>("/RAWItongueOut", 1, itongue_callback);
    ros::Subscriber vision_sub = nh.subscribe<vision::Detection_array>("/Vision/ObjectDetection", 1, vision_callback);
